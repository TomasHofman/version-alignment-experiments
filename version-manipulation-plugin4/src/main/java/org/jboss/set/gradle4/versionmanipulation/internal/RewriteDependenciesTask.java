 ...
 *
 * Deprecated: This is however a dead end, because an attempt to modify DependencySet of given configuration will fail,
 * if that configuration has already been resolved. Configuration may be resolved e.g. by the build script calling
 * something like `configurations.compile.each {...}`. The ability to modify dependencies therefore cannot be relied on.
 */
@Deprecated
